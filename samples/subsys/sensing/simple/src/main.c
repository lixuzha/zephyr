...
static void acc_data_event_callback(sensing_sensor_handle_t handle, const void *buf)
{
	...
	struct sensing_sensor_value_3d_int32 *sample = (struct sensing_sensor_value_3d_int32 *)buf;

	LOG_INF("%s(%d), handle:%d, Sensor %s data:\t x: %d, y: %d, z: %d",
...
void main(void)
{
	...
	if (ret) {
		LOG_ERR("sensing_close_sensor:%d error:%d", base_acc, ret);
	}

	ret = sensing_close_sensor(lid_acc);
	if (ret) {
		LOG_ERR("sensing_close_sensor:%d error:%d", lid_acc, ret);
	}
}
