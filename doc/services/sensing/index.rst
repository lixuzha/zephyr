...
leveraging Zephyr's large library of sensor device drivers (100+).
...
* Sensing Subsystem
    ...
    * Sensing API (:zephyr_file:`include/zephyr/sensing/sensing_sensor.h`)
...
* Enumerate all sensor instances
  ...
  dynamic enumerate underlying platforms' sensor instances.
...
Clients using a ``void*`` type handler to handle a opened sensor
...
.. note::
   ...
   Application(s) need call sensing_sensor_open() APIs to explicitly open the sensor instance.
...
* Data Structure
  ...
  Each sensor sample value defined as a common ``header`` + ``readings[]`` data structure:

  .. code-block:: c

      struct sensing_sensor_value_xxx {
...
* Time Stamp
  ...
  The ``header`` defined a **base_timestamp**, and
  each element in **readings[]** array defined **timestamp_delta**.
...
* Data Format
  ...
  :c:macro:`sensing_sensor_value_uint32` can be used by
  :c:macro:`SENSING_SENSOR_TYPE_LIGHT_AMBIENTLIGHT` sensor,

  and :c:macro:`sensing_sensor_value_q31` can be used by
