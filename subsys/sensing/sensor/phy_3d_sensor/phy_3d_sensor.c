...
static q31_t scaled_int64_to_shifted_q31(int64_t val, int64_t scale,
		int8_t shift)
{
	return (q31_t)((val * BIT(31) / scale) >> shift);
}

static q31_t accel_sensor_value_to_q31(struct sensor_value *val)
{
	int64_t micro_g = sensor_ms2_to_ug(val);
...
static void accel_q31_to_sensor_value(q31_t q, struct sensor_value *val)
{
	...
	sensor_ug_to_ms2((int32_t)micro_g, val);
}
...
{
	int64_t micro_deg_x10 = (int64_t)sensor_rad_to_10udegrees(val);

	return scaled_int64_to_shifted_q31(micro_deg_x10, 100000LL,
...
static void gyro_q31_to_sensor_value(q31_t q, struct sensor_value *val)
{
	...
	sensor_10udegrees_to_rad((int32_t)(micro_deg / 10), val);
}
